...
| - | 设备状态心跳 | ✅（设置页"测试连接"） |
| - | **蓝牙直连发消息** | ✅（消息页"蓝牙直连"开关：无需WiFi，扫描连接 JIUJIU 直接写入消息；2026-08-23 新增） |
...
> v0.2.0（2026-08-23）：消息页新增 **蓝牙直连** 开关。开启后发消息走 BLE 直达 JIUJIU（无需 WiFi/热点），
> 首次使用会请求蓝牙权限；关闭则走原有 HTTP 通道。APK 构建：`gradlew assembleDebug`（产物在 `app/build/outputs/apk/debug/`）。
