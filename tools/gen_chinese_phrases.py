# -*- coding: utf-8 -*-
"""生成固定中文陪伴短句的 24x24 单色字模头文件，供 TFT_eSPI drawBitmap 使用。"""
...
def pack_glyph_row(image: Image.Image, y: int, width: int) -> bytes:
    data = bytearray()
    byte = 0
    bit_index = 0
...
def render_phrase(phrase: str) -> bytes:
    ...
    for index, char in enumerate(phrase):
        ...
        draw.text((x + (GLYPH_W - char_w) / 2 - bbox[0], (GLYPH_H - char_h) / 2 - bbox[1]), char, font=font, fill=255)
    out = bytearray()
...
def main() -> None:
    ...
    lines.append("")
    lines.append("struct ZhPhrase { const uint8_t* data; uint16_t w; };")
    lines.append("static const ZhPhrase zh_phrases[ZH_PHRASE_COUNT] = {")
    for index, name in enumerate(names):
        lines.append(f"  {{ {name}, {name.upper()}_W }},")
    lines.append("};")
    lines.append("")
    lines.append("#endif")
    OUT_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print("generated", OUT_PATH, "chars:", " ".join(PHRASES))
